...
double findAguPath(int cntPt, int endPt, double minCap)
{
    if (cntPt == endPt)
        return minCap;
    ...
    for (int & i = lastVisitedEdge[cntPt]; i != -1; i = arr[i].next)
    {
        ...
        {
            double flowInc = findAguPath(nextPt, endPt, min(minCap - cntFlow, arr[i].capacity));
            if (fabs(flowInc) <= eps)
            {
                depth[nextPt] = -1;
            }
...
double dinic(int startPt, int endPt)
{
    ...
    while (updateDepth(startPt, endPt))
    {
        ...
        double flowInc =  findAguPath(startPt, endPt, DBL_MAX);
        if (fabs(flowInc) <= eps)
            break;
        ans += flowInc;
    }
